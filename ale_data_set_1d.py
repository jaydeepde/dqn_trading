...
actions, and rewards.
    ...
    """
    def __init__(self, width, rng, max_steps=1000, phi_length=4):
        """Construct a DataSet.
        ...
        self.imgs = np.zeros((max_steps, width), dtype='float32')
    ...
    def phi(self, img):
        ...
        phi = np.empty((self.phi_length, self.width), dtype=floatX)
...
def simple_tests():
    np.random.seed(222)
    dataset = DataSet(width=3,
                      rng=np.random.RandomState(42),
                      max_steps=6, phi_length=4)
    for i in range(10):
        img = np.random.randint(0, 256, size=(3,)).transpose()
...
def speed_tests():
    ...
    img = np.random.randint(0, 256, size=(80,).transpose())
    action = np.random.randint(16)
    reward = np.random.random()
    start = time.time()
...
def trivial_tests():
    ...
    img1 = np.array([[1, 1]], dtype='float32')
    img2 = np.array([[2, 2]], dtype='float32')
    img3 = np.array([[3, 3]], dtype='float32')
...
def test_memory_usage_ok():
    ...
    for i in xrange(1000000000):
        if (i % 100000) == 0:
            print i
        dataset.add_sample(np.random.random((80,).transpose()), 1, 1, False)
        if i > 200000:
